...
{
#if PLATFORM_WINDOWS
	// Blindaje: si Spout.dll no se pudo cargar explicitamente en
	// FSpoutModule::StartupModule (ver el comentario ahi), ni SpoutDirectX
	// ni spoutSenderNames se pueden construir sin arriesgar el
	// ERROR_MOD_NOT_FOUND sin capturar que tumbaba el editor. Se corta aca,
	// antes de tocar nada de eso, y se avisa una sola vez (D3D11Device
	// nunca queda seteado en este camino, asi que InitializeIfNeeded()
	// vuelve a llamar Initialize() en cada Tick mientras Spout no este
	// disponible -- bLoggedOnce evita repetir el aviso cada vez).
	if (!FSpoutModule::IsSpoutRuntimeAvailable())
	{
		...
		if (!bLoggedOnce)
		{
			UE_LOG(LogSpoutPlugin, Error,
				TEXT("Spout.dll no esta disponible en este proceso: se omite toda inicializacion de ")
				TEXT("D3D/Spout. Revisar el log de arranque del plugin (categoria LogSpoutPlugin) para el motivo real."));
			bLoggedOnce = true;
		}
		return;
	}
